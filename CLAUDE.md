...
2. **A `## References` list** below the matrix — *primary research only*:
   ...
   - If the publisher has issued a **post-publication notice that leaves the paper standing** (a correction, an erratum, an expression of concern), record it the same way, under its own DOI. **Use the publisher's own word for the notice as the label** (`Correction`, `Erratum`, `Expression of concern`), never a generic one:
     ...
     **The blockquote is DB-owned. Author it through `caail-db-authoring`, never by typing into `Papers.md`.** `db:emit` rebuilds every trailing blockquote from `papers.blockquotes_md`, so a line typed into the Markdown is either deleted by the next emit or reddens the CI sync guard. `block-generated-edits.py` will not catch an `Edit` that only touches the blockquote: its markers are `<a id="`, `](#` and `### [`, and such an edit contains none of them, so the local guard that exists for this file stays silent. (A whole-file `Write` is denied regardless of content, so only the Edit-shaped path is exposed.) `site/scripts/db/hook.test.ts` pins that gap, so closing it turns the suite red and forces this sentence to be rewritten rather than left asserting the opposite.
