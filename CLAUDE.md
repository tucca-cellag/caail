...
License: <optional — SPDX id or short note, e.g. "Non-commercial">
```
...
- **License field (optional).** Each entry carries a coarse license *triage tag* (a detected SPDX id or a curated note), surfaced on the site as a colored badge (permissive / copyleft / restricted / unknown) and rolled up into a distribution on the By the Numbers dashboard. For **GitHub-hosted software the license is auto-detected** — `pnpm --dir site fetch:licenses` writes the committed `site/scripts/parser/license-cache.json` (SPDX per repo), which `parse`/`build` fold in offline (network-free, exactly like `fetch:citations`/`fetch:awesome-lists`). Add an explicit `License:` line to the entry (its own paragraph, parsed like `Summary:` and stripped from the rendered summary) only when there's nothing to auto-detect — a non-GitHub resource or a repo GitHub can't classify (custom/non-commercial). The manual line **wins** over the cache. The tag is a triage signal, not the terms: it is not a substitute for reading the actual license (consistent with the "fetch canonical" note atop `Software.md`/`Databases.md`). The tier classifier (`site/src/lib/licenses.ts`) is shared by the badge, the dashboard, and the metrics rollup so they can't disagree.
