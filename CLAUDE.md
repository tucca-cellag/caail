...
- **Stack:** Astro + Starlight, Preact islands, `astro-icon` (Phosphor icon set), self-hosted fonts via `@fontsource` (Bricolage Grotesque for display, Inter for body, JetBrains Mono for code/identifiers), OKLch design tokens, `lite-youtube-embed` for talk facades. The design system is documented in the repo-root `DESIGN.md`.
...
- **Scope/branching:** the site is built one milestone at a time in a worktree off `main`, PR'd back. Issue #13 tracks the full plan (milestones M0–M7): M0 prototype, M1 parser+lint, M2 core site + deploy, and M3 catalog browsers (Software/Databases) + Talks are in place; the Citation Network (M5/#8) and Metrics Dashboard (M6/#9) remain. Routes: Home, Papers Explorer, Software, Databases, Datasets (by species), Research Areas, Talks, Contributing, About.
