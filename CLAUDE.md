...
**Rendering.** Unlike the other canonical prose files, primers are **not** served through the prose loader. A dedicated parser (`site/scripts/parser/primers.ts` → `primers.json`) classifies each item and rewrites internal `.md` links to site routes (Papers→`/papers/explorer/`, Talks/OtherResources/ResearchAreas→their routes). A canonical-prose target **keeps** its section `#anchor`; a card or island route (the `DEDICATED_ROUTES` in `site/src/content/dedicated-routes.ts`) keeps one only when `site/scripts/dedicated-links.ts` can show the anchor exists there, and otherwise falls back to the GitHub blob. `PrimerHub.astro` renders the result at `/primers/cell-ag/` and `/primers/ai/`: YouTube videos embed inline (`lite-youtube`), playlists/external links become cards, internal links become same-tab CAAIL nav cards. So the canonical Markdown stays the source of truth (and is in `llms-full.txt`) while the site upgrades it to playable, navigable pages. Anchor gotcha: `/talks/` and primer heading ids use the single-dash `siteSlug` (`site/src/lib/heading-slug.ts`, `AI/ML`→`ai-ml`), while canonical-prose pages (OtherResources) use the GitHub/Starlight double-dash-for-`&` slugger. For Talks and the primers either form resolves, since `dedicated-links.ts` maps both, and an anchor naming no section fails the build; for a canonical-prose page, match its scheme.
