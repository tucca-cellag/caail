...
1. **A 2D matrix table** at the top, for **primary research** applying a specific AI method to a specific cell-ag problem:
   ...
   - **Columns** = research area, each linked to its definition in `Taxonomy.md`. Current columns: Media Optimization, Cellular Engineering, Bioprocess & Scale-Up, Scaffolding, Sensory Prediction, Metabolic Modeling, Food Safety Prediction, **AI Tooling / Methodology**.
...
- **`papers.ndjson` holds six sections, not two:** References (230), Reviews & Perspectives (76), and four `… Reference Work` sections (42). Only the 230 are matrix-eligible, so label every count with the population it counted — "348 papers" and "papers in the matrix" are different numbers.
...
- **Commands:** `pnpm --dir site dev` (local preview at `/`), `pnpm --dir site build` (runs the parser first), `pnpm --dir site test` (vitest parser suite), `pnpm --dir site test:e2e` (Playwright + axe a11y), `pnpm --dir site parse` (regenerate data only).
...
## The Claude Code plugins (`plugin/`, `plugin-contribute/`)
...
**`caail-contribute` carries `disallowed-tools`, which is a Claude Code-only field and therefore pins its distribution.** It removes the writing and delegation tools while the skill is active, so the "it never files anything" promise on the marketplace entry has something behind it besides prose. **Which tools those are lives in the frontmatter and deliberately not here**, for the same reason the pick-by-hand list is not restated below: this line already went stale once, naming four while the field listed five, and the missing one was `Agent` — the entry that does the most work, since a subagent does **not** inherit its parent's restrictions, so leaving `Agent` callable means a prompt-injected paper can delegate the `gh` call to a fresh agent that still has `Bash`. Read the frontmatter. Two things follow. The **claude.ai upload / Skills API path accepts only six frontmatter fields** (`name`, `description`, `license`, `compatibility`, `metadata`, `allowed-tools`) and fails with a hard `Unexpected key(s) in SKILL.md frontmatter` error on anything else, so this skill cannot be packaged that way while the field is present; it ships through the Claude Code marketplace and that is fine. And **do not "fix" it to `allowed-tools`**: that field *grants* pre-approval and explicitly does not restrict anything ("every tool remains callable"), so the swap would read as a tightening and be a widening. A draft of this branch made exactly that mistake and asserted it had made the guarantee structural.

**`caail-contribute` composes GitHub issue URLs, and the parameter list in its SKILL.md is a checked contract, not prose.** GitHub prefills an issue form by matching a query parameter to a field's `id` and reports nothing when the match fails, so a renamed field, or a field converted to a `dropdown` (which never prefills at all), arrives blank while the agent says it filled the form in. `site/scripts/parser/contribute-form.ts` therefore READS the parameter list out of the skill and reconciles it against `.github/ISSUE_TEMPLATE/*.yml` at parse time, failing the build on drift; `contribute-form.test.ts` covers it, and both `plugin-contribute/**` and `.github/ISSUE_TEMPLATE/**` are in `test.yml`'s path filters so a one-sided edit still runs it. Sibling of `correction-form.ts`, which does the same for `/report/`. Two consequences: **`title` is not a usable field id** (GitHub reserves it for the issue title, which is why `paper.yml` says `paper_title`), and **a `dropdown` never prefills at all**, so the skill's job is to tell the contributor which ones to pick. *Which* fields those are lives in the skill's own pick-by-hand list and deliberately nowhere else, this line included: an earlier draft enumerated them here and in the skill, and the enumeration is what `assertRequiredCovered` exists to make unnecessary. A copy here would go on naming three after a fourth landed, in a file that loads every session.

**Neither `plugin.json` carries a `version` field, deliberately. Do not add one to either.** Claude Code resolves a plugin's version from the first of: `plugin.json`'s `version`, the marketplace entry's `version`, then the source's git commit SHA — and it uses that as the cache key deciding whether an update exists. An explicit `version` therefore *pins* the plugin: users receive updates only when the field is bumped, and `/plugin update` reports "already at the latest version" no matter how many commits have landed. That is precisely what went wrong here — `0.1.0` never moved while `SKILL.md` changed three times, so every installed user sat on the original skill and had no way to know. Omitting the field falls through to the commit SHA, which is the documented model for a plugin under active development, and CAAIL's skill is exactly that: it restates endpoint shapes and corpus counts that move with the data.

`claude plugin validate .` warns "No version specified. Consider adding a version following semver." once per plugin, so it now prints twice. **That warning is expected and should be ignored here** — acting on it silently restores the delivery bug. If CAAIL ever does want release cadence over freshness, the change is to re-add `version` *and* adopt a bump-on-every-skill-edit discipline (plus a `CHANGELOG.md`); half of that is worse than neither. `metadata.version` in `marketplace.json` is the marketplace's own version and is not in the plugin's resolution chain, so it is unaffected.
