...
### [AllerTrans](https://github.com/faezesarlakifar/AllerTrans)

Summary: Predicts whether a protein sequence is allergenic by encoding it with two protein language models, ESM-2 and ProtT5, concatenating their embedding vectors, and classifying the result with a deep neural network; the released model ensembles the top-performing variants to balance sensitivity against specificity. It reports 97.91% sensitivity, 97.69% specificity and 97.80% accuracy under 5-fold cross-validation on AlgPred 2.0’s own training and validation split, the corpus catalogued in [Datasets/FoodSafety.md](./Datasets/FoodSafety.md), so those figures are not independent of the AlgPred 2.0 entry above. Because it learns from pretrained sequence representations rather than homology to a known allergen, it is applicable to the recombinant growth factors, media proteins and scaffold proteins introduced into cultured products, which often have limited similarity to catalogued allergens. A hosted demo runs as a [Hugging Face Space](https://huggingface.co/spaces/sfaezella/AllerTrans); companion to [Papers.md ref #289](./Papers.md#289).

### [NetAllergen](https://services.healthtech.dtu.dk/services/NetAllergen-1.0/)

Summary: Random-forest allergenicity predictor from DTU Health Tech that introduces MHC class II peptide presentation propensity, computed for HLA-DR and HLA-DQ alleles, as a feature alongside conventional physicochemical descriptors. It was trained on IgE-inducing allergens from AllergenOnline after redundancy removal through a protein-partitioning pipeline. Its reported advantage is specifically on proteins with limited sequence similarity to known allergens, where it outperforms a BLAST baseline and AlgPred 2. That is the regime that matters for novel cell-ag proteins, which are precisely the ones homology screens handle worst.

### [ChAlPred](https://webs.iiitd.edu.in/raghava/chalpred/)

Summary: Predicts the allergenic potential of **chemical compounds** rather than proteins, filling a gap the sequence-based allergen predictors do not cover. It was trained on 403 allergenic and 1,074 non-allergenic compounds from IEDB, using PaDEL-computed 2D, 3D and fingerprint molecular descriptors across several machine-learning classifiers, and it also generates analogs with a desired allergenicity. For cellular agriculture the relevant surface is the small-molecule side of a formulation (media additives, supplements, process residues and cryoprotectants), which protein allergenicity screens cannot assess.
