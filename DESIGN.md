...
- **Favicon package** — a simplified bioreactor glyph (vessel, lid, stir shaft + impeller, three scattered cells), all the same mark: `favicon.svg` is drawn in `currentColor` with a `prefers-color-scheme` flip (Tufts navy on light, white on dark) for modern browsers; the raster fallbacks for everything else (`favicon.ico` 16/32, `apple-touch-icon.png` 180, manifest `icon-192.png`/`icon-512.png` maskable) are a **white glyph on a navy field** — fixed-color since rasters can't flip, so they stay visible on light *and* dark tab bars. `site/public/site.webmanifest` (theme/background `#002E6D`, `start_url`/`scope` `/caail/`) plus a `theme-color` meta complete the set. The rasters are generated from `favicon.svg` (the glyph source of truth — recolored, not re-drawn) by `site/scripts/favicons.mjs` (run once via `node scripts/favicons.mjs`; outputs committed). Cells are scattered (no bilateral symmetry) to avoid a face-like read.
